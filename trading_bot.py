...
Bet sizing uses fractional Kelly Criterion:
    f_full    = (p - a) / (1 - a)
    f_scaled  = kelly_fraction * f_full
    contracts = floor(f_scaled * balance_dollars / a),  min 1

Designed to run alongside KalshiCollector and BtcPriceTracker via callbacks.
"""
...
from kalshi_orderbook_collector import KalshiConfig, KalshiSigner, discover_btc_15m_markets
from market_discovery import btc_sigma_in_window
from true_prob import yes_probability

WINDOW_S: float = 60.0
EDGE_THRESHOLD: float = 0.12
DEFAULT_KELLY_FRACTION: float = 1
DEFAULT_SIGMA_FALLBACK: float = 0.3
BALANCE_PRINT_INTERVAL_SEC: float = 300.0  # 5 minutes
...
def _trapezoid(pts: list[tuple[float, float]], t_lo: float, t_hi: float) -> float:
    ...
    Copied from plot_market.py to avoid the matplotlib.use() import side-effect.
    """
    A = 0.0
...
class MarketState:
    ticker: str
    K: float        # floor_strike USD
    T_ms: float     # close_time as Unix ms
    sigma: float    # USD/sqrt(s)
    ...
    def update_btc(self, price: float, ts_ms: int) -> None:
        """
        Maintain the running window_pts list for A_t computation.
        Mirrors the anchoring fix in compute_true_probs() (plot_market.py):
        prepend (window_open_sec, last_known_price) on the first inside-window
        tick so A_t doesn't start at 0.
        """
        t_sec = ts_ms / 1000.0
...
class KalshiTrader:
    def __init__(self, config: KalshiConfig, kelly_fraction: float = DEFAULT_KELLY_FRACTION):
        ...
        # Keys are "ticker:yes" or "ticker:no" to track each side independently
    ...
    # ── callbacks (synchronous, called from event loop) ─────────────────────
    ...
    def on_book_state(self, record: dict[str, Any]) -> None:
        ...
        # Update cached asks from record
        ...
        # YES side
        if not state.has_yes_bet and f"{ticker}:yes" not in self._pending_orders:
            ask = state.latest_yes_ask
            if ask is not None and (p_yes - ask) >= EDGE_THRESHOLD:
                logging.info("YES edge: %s  p=%.4f  ask=%.4f  edge=+%.4f",
                             ticker, p_yes, ask, p_yes - ask)
                self._pending_orders.add(f"{ticker}:yes")
                asyncio.create_task(self._handle_signal(ticker, p_yes, ask, "yes"))

        # NO side
        if not state.has_no_bet and f"{ticker}:no" not in self._pending_orders:
            ask = state.latest_no_ask
            if ask is not None and (p_no - ask) >= EDGE_THRESHOLD:
                logging.info("NO edge:  %s  p=%.4f  ask=%.4f  edge=+%.4f",
                             ticker, p_no, ask, p_no - ask)
                self._pending_orders.add(f"{ticker}:no")
    ...
    def _kelly_contracts(self, p: float, a: float, balance_cents: int) -> int:
        if a <= 0.0 or a >= 1.0:
            return 1
        f_full = (p - a) / (1.0 - a)
        f_scaled = self.kelly_fraction * f_full
        if f_scaled <= 0.0:
            return 1
        balance_dollars = balance_cents / 100.0
        return max(1, math.floor(f_scaled * balance_dollars / a))
    ...
    async def _place_order(self, ticker: str, price_cents: int, count: int, side: str) -> bool:
        ...
        url = self.config.rest_base_url + "/portfolio/orders"
    ...
    async def _handle_signal(self, ticker: str, p: float, ask: float, side: str) -> None:
        try:
            ...
            if state is None:
                return
            if side == "yes" and state.has_yes_bet:
                return
            if side == "no" and state.has_no_bet:
                return

            # Re-check edge in case ask moved since callback fired
            ...
            if current_ask is None or (p - current_ask) < EDGE_THRESHOLD:
                logging.info("Edge evaporated for %s %s before order placed", side.upper(), ticker)
                return

            price_cents = max(1, min(99, round(current_ask * 100)))
            balance_cents = await self.fetch_balance()
            if balance_cents <= 0:
                logging.warning("Zero balance, skipping %s order for %s", side.upper(), ticker)
                return

            count = self._kelly_contracts(p, current_ask, balance_cents)
            logging.info(
                "Placing %s order: %s  p=%.4f  ask=%.4f  edge=+%.4f  count=%d  balance=$%.2f",
                side.upper(), ticker, p, current_ask, p - current_ask, count, balance_cents / 100.0,
            )
            ...
            self._pending_orders.discard(f"{ticker}:{side}")
        ...
        for m in markets:
            ...
            if prev_open_ms is not None and prev_close_ms is not None and btc_file.exists():
                ...
                if sigma <= 0.0:
                    ...
                    logging.warning("Sigma=0 for %s prev window, using fallback %.1f", ticker, sigma)
            else:
                sigma = DEFAULT_SIGMA_FALLBACK
            ...
            if open_time:
                prev_open_ms = _iso_to_ms(open_time)
            prev_close_ms = T_ms
